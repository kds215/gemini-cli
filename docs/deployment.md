...
- `@kds215/gemini-cli-core`: The backend, handling logic and tool execution.
...
- **NPM publication:** For publishing to the NPM registry, the TypeScript source code in `@kds215/gemini-cli-core` and `@google/gemini-cli` is transpiled into standard JavaScript using the TypeScript Compiler (`tsc`). The resulting `dist/` directory is what gets published in the NPM package. This is a standard approach for TypeScript libraries.
